...
exports.normalizeExcelTime = void 0;
...
exports.normalizeExcelTime = normalizeExcelTime;
/**
...
function parseExcelRows(rows, options) {
    ...
    if (bibIdx === -1)
        warnings.push('No se encontro columna de Dorsal (Bib). Usando columna 1 como fallback.');
    if (timeIdx === -1 && statusIdx === -1)
        warnings.push('No se encontro columna de Tiempo ni Estado.');
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (!name) {
            warnings.push(`Fila ${i + 1}: sin nombre. Omitida.`);
            continue;
        }
        // Tiempo / Estado
        ...
        if (statusIdx !== -1) {
            ...
            if (DNF_FLAGS.includes(statusVal))
                timeRaw = statusVal;
        }
        const time = (0, exports.normalizeExcelTime)(timeRaw);
        if (!time) {
            warnings.push(`Fila ${i + 1} (${name}): tiempo "${timeRaw}" no reconocido. Omitida.`);
            continue;
        }
