...
export const CATEGORY_KEYWORDS = ['MASTER', 'ELITE', 'NOVICIO', 'DAMAS', 'VARONES', 'MIXTO', 'PRO', 'INFANTIL', 'JUVENIL', 'CADETE', 'SUB', 'EBIKE', 'ENDURO'];
...
export function parseResultsText(fullText: string, fallbackCategory: string = 'DESCONOCIDA') {
  if (!fullText || !fullText.trim()) return [];

  const categoryMarkers: { index: number; category: string }[] = [];
